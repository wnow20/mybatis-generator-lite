...
public abstract class AbstractXmlElementGenerator extends AbstractGenerator {
    ...
    public void createPageElement(XmlElement parent) {

        if (context.getEnablePage()) {
            XmlElement pageElement = new XmlElement("if");
            pageElement.addAttribute(new Attribute("test", "rows != null"));
            pageElement.addElement(new TextElement("limit ${offset}, ${rows}"));
            parent.addElement(pageElement);
        }
    }
}
